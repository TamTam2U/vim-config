...
-- Buffer
-- Close buffer
set({ "n" }, "<leader>wd", ":bd<CR>", { noremap = true, desc = "Close Buffer" })
-- Close All buffer
set({ "n" }, "<leader>wD", ":%bd|e#|bd#<cr>", { noremap = true, desc = "Close Buffer" })

-- Buffer navigation
-- Next buffer
set({ "n" }, "<leader>wn", ":bnext<CR>", { noremap = true, desc = "Next Buffer" })

-- Previous buffer
set({ "n" }, "<leader>wp", ":bprevious<CR>", { noremap = true, desc = "Previous Buffer" })

-- run javascript file
vim.cmd([[
  autocmd FileType javascript nnoremap <buffer> <F5> :!node %<CR>
  autocmd FileType typescript nnoremap <buffer> <F6> :!ts-node %<CR>
]])
